...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (canvas && motionEnabled) {
        ...
        class Particle {
            constructor() {
                ...
                this.color = `rgba(${Math.random() > 0.5 ? '255, 221, 53' : '175, 82, 222'}, ${Math.random() * 0.5 + 0.3})`;
            }
            ...
            for (let i = 0; i < particles.length; i++) {
                for (let j = i + 1; j < particles.length; j++) {
                    ...
                    if (distance < 120) {
                        ...
                        ctx.strokeStyle = `rgba(255, 221, 53, ${opacity})`;
                        ctx.lineWidth = 0.5;
                        ctx.beginPath();
    ...
    // ============ Contact Form ============
    if (contactForm) {
        const scriptURL = 'https://script.google.com/macros/s/AKfycbxM_tIu2ZgudX3oogFtXbxxZ1qmqE87kkv8syIsWZ0eue-k7v-EwttEyyzkaZHeBYphOw/exec';
    ...
    // ============ Tech Stack Progress Bar Animation (global, excluding AI Tech Expertise section) ============
    const allTechProgressBars = document.querySelectorAll('.tech-progress-fill');
    const techProgressBars = Array.from(allTechProgressBars).filter(bar => !bar.closest('.tech-stack-section'));
    ...
    // Trigger animation on scroll and load for non-tech-stack sections only
    if (techProgressBars.length){
      window.addEventListener('scroll', animateTechProgress);
      window.addEventListener('load', animateTechProgress);
      setTimeout(animateTechProgress, 500);
    }
      ...
      const observer = new IntersectionObserver((entries, obs) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            const fill = entry.target.querySelector('.tech-progress-fill');
            if (fill) {
              let value = fill.getAttribute('data-progress') || fill.dataset.progress || fill.getAttribute('aria-valuenow') || '0';
